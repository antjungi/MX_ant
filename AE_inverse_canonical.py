...
# -*- coding: utf-8 -*-
"""
DeepCAD AE + Return-3 Common-Curve Residual Surrogate  —  canonical ordering
============================================================================

canonical 변경점 (base = AE_inverse.py):
  - 데이터 전처리 단계에서 ★ sketch 청크 (SOL ... EXT) 순서를 canonical 정렬 ★
  - 정렬 키: (z 위치 asc, footprint area desc, x asc, y asc)
      · 1차: 낮은 z (보통 GND/substrate 같은 바닥 layer)
      · 2차: 같은 z 안에선 footprint 면적 큰 것부터
      · 3차/4차: 위치로 tie-break
  - JSON real coord 기반 (시각화와 동일 좌표계)
  - 청크 내부 토큰 순서는 그대로 유지 (autoregressive 학습 안정)
  - 모델 / loss / 인버스 디자인 코드는 base 와 동일

목표:
  "비슷한 위치·크기의 sketch 는 비슷한 sequence 위치" 만들어
  type 1/2/3 간 sketch order mismatch 제거 → autoregressive decoder 가
  position 별로 일관된 의미 학습 가능 → inverse design 시 안정적 출력.
"""
...
class JointDataset(Dataset):
    def __init__(
        ...
        if self.do_canonicalize:
            print(
                f"  ★ canonical: reordered {n_changed}/{n_total_canon} sequences "
