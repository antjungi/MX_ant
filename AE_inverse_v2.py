...
v2 변경점 (v1 = AE_inverse.py):
  - 학습 데이터의 sketch 순서를 ★ canonicalize ★
    정렬 키 = (부피 desc, z asc, x asc, y asc)
    → 모든 sample 이 "큰 sketch 먼저, 그 다음 위치 순" 으로 통일
    → type 1/2/3 간 sketch order mismatch 제거
    → Decoder 가 일관된 의미적 순서로 학습됨
  - 모델 구조 / loss / 최적화는 v1 과 동일 (변수 통제용)
...
class CFG:
    ...
    # ★ v2: sketch 순서 canonicalize (volume desc, z asc, x asc, y asc)
...
def train_fixed_medium_var(
    ...
    print(f"  Canonicalize        : {getattr(cfg, 'canonicalize_sketches', True)} "
          f"(volume desc, z asc, x asc, y asc)")
