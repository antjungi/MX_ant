...
# -*- coding: utf-8 -*-
"""
DeepCAD AE + Return-3 Common-Curve Residual Surrogate  —  shuffle aug version
=============================================================================

shuffle 변경점 (base = AE_inverse.py):
  - 학습 시 sketch 청크 (SOL ... EXT) 순서를 ★ 매 sample 마다 무작위 셔플
  - Encoder 입력 == Decoder target (둘 다 같은 셔플)
  - 청크 내부 토큰 순서는 그대로 유지
  - 효과적으로 sample 당 N! 종류의 학습 데이터 → augmentation
  - 같은 sample 의 여러 셔플 버전이 비슷한 z 로 매핑되어
    latent 클러스터가 두꺼워지고 사이 빈 공간이 줄어듦
    → inverse design 시 z 가 빈 공간으로 새는 문제 완화 기대
...
class CFG:
    ...
    # ★ shuffle aug: encoder 입력 + decoder target 양쪽에 같은 sketch 청크 셔플
    shuffle_sketch_aug: bool = True
    shuffle_aug_prob: float = 0.5      # 매 sample 마다 이 확률로 셔플 (나머지는 원본)
    shuffle_aug_seed: int = 1234
...
def shuffle_sketch_chunks(tokens, rng):
    """sketch 청크 (SOL ... EXT) 들의 순서를 무작위 셔플.
    - 청크 내부 토큰 순서는 그대로 유지
    - EOS / padding 은 tail 로 보존
    - rng: numpy.random.Generator

    이 함수는 ENCODER 입력과 DECODER target 양쪽에 같은 결과를 적용해
    autoregressive 학습 안정성을 유지함 (옵션 B).
    """
    ...
    out = np.concatenate(
        shuffled + ([tail] if tail.size else []), axis=0,
    )
...
class JointDataset(Dataset):
    def __init__(
        ...
        type_ids=None,
        type_names=None,
        shuffle_sketch_aug=False,
        shuffle_aug_prob=0.5,
        shuffle_aug_seed=1234,
    ):
        self.npy_files = list(npy_files)
        self.max_len = max_len

        # ★ shuffle aug 설정
        self.shuffle_sketch_aug = bool(shuffle_sketch_aug)
        self.shuffle_aug_prob = float(shuffle_aug_prob)
        self.aug_rng = np.random.default_rng(int(shuffle_aug_seed))
    ...
    def get_item_with_aug(self, idx, augment=False):
        """augment=True 일 때 sketch 청크 셔플 (옵션 B: enc/dec 동일 셔플)."""
        tok = self.padded[idx]
        if (
            augment
            and self.shuffle_sketch_aug
            and self.aug_rng.random() < self.shuffle_aug_prob
        ):
            # padded 가 float32 라 int32 로 변환 → 셔플 → 다시 float32
            t_int = tok.astype(np.int32)
            t_int = shuffle_sketch_chunks(t_int, self.aug_rng)
            tok = t_int.astype(np.float32)
        return (
            torch.tensor(tok, dtype=torch.float32),
...
def load_multitype_data(cfg, script_dir):
    ...
    dataset = JointDataset(
        ...
        shuffle_sketch_aug=getattr(cfg, "shuffle_sketch_aug", True),
        shuffle_aug_prob=getattr(cfg, "shuffle_aug_prob", 0.5),
        shuffle_aug_seed=getattr(cfg, "shuffle_aug_seed", 1234),
    )
...
def train_fixed_medium_var(
    ...
    # ★ shuffle aug: train 만 적용, val 은 결정적 평가 위해 OFF
    train_set = SubsetDataset(dataset, train_idx, augment=True)
    ...
    print(f"  Shuffle sketch aug  : "
          f"{getattr(cfg, 'shuffle_sketch_aug', True)} "
          f"(prob={getattr(cfg, 'shuffle_aug_prob', 0.5)})")
