...
# -*- coding: utf-8 -*-
"""
DeepCAD AE + Return-3 Common-Curve Residual Surrogate  —  role + variant snap
=============================================================================
...
요구사항:
  ...
  - sklearn 없으면 자동 fallback (canonical 순서 안 함, variant snap 도 안 함)
"""
...
# ══════════════════════════════════════════════════════════════
# ★ Role discovery: per-sketch feature extraction + k-means clustering
# ══════════════════════════════════════════════════════════════
def _extract_sketch_features_from_json(json_data):
    """각 sketch 의 feature vector (clustering 용).

    Feature (8 차원):
      [0] x_centroid   (mm)
      [1] y_centroid   (mm)
      [2] z_centroid   (mm)
      [3] footprint_area (mm²)  — bbox 큰 두 dim 곱
      [4] log_area = log(area+1)
      [5] bbox_dim_max (mm)     — bbox 가장 긴 축
      [6] bbox_dim_mid (mm)     — 두 번째 긴 축
      [7] bbox_dim_min (mm)     — 가장 짧은 축 (sketch plane normal 방향에 대응)

    Returns: list of np.array (one per sketch) 또는 None / [None, ...]
    """
    try:
        sketches = json_to_real_sketches(json_data)
    except Exception:
...
def build_role_clusters(all_sample_features, n_clusters=8, random_state=0, verbose=True):
    """모든 sample 의 sketch features 를 한꺼번에 k-means clustering.

    Args:
        all_sample_features: list (len=N_samples), 각 원소는 list of feature vecs (or None)
        n_clusters: K
        verbose: cluster 별 요약 print

    Returns:
        per_sample_role_ids: list (len=N_samples), 각 원소는 list of role IDs (or None)
        cluster_centers: (K, F) numpy or None
    ...
    try:
        ...
        if verbose:
            print("  ⚠ sklearn 미설치 — role clustering skip (pip install scikit-learn)")
        return [None] * len(all_sample_features), None, None

    # Flatten all feats
    flat_feats = []
    map_back = []   # (sample_idx, sketch_idx)
    for s_idx, sample_feats in enumerate(all_sample_features):
        if sample_feats is None:
            continue
        for k, f in enumerate(sample_feats):
            if f is None:
                continue
            flat_feats.append(f)
            map_back.append((s_idx, k))

    n_total = len(flat_feats)
    if n_total < n_clusters:
        if verbose:
            print(f"  ⚠ too few sketches ({n_total}) for K={n_clusters} — skip clustering")
        return [None] * len(all_sample_features), None, None

    X = np.array(flat_feats, dtype=np.float32)
    ...
    # Build per-sample roles
    per_sample_roles = [None] * len(all_sample_features)
    for (s_idx, k), rid in zip(map_back, labels):
        if all_sample_features[s_idx] is None:
            continue
        if per_sample_roles[s_idx] is None:
            per_sample_roles[s_idx] = [None] * len(all_sample_features[s_idx])
        per_sample_roles[s_idx][k] = int(rid)

    if verbose:
        # cluster 별 요약 + 대표 sample 출력
        print(f"\n  ★ Role discovery: K={n_clusters} clusters, fit on {n_total} sketches")
        print(f"  {'cluster':>7s} {'count':>6s} {'x_cen':>7s} {'y_cen':>7s} {'z_cen':>7s} "
              f"{'area':>10s} {'bbox_max':>9s} {'bbox_mid':>9s} {'bbox_min':>9s}")
        print("  " + "-" * 78)
        for c in range(n_clusters):
            ...
            print(
                ...
                f"{m[3]:>10.1f} {m[5]:>9.2f} {m[6]:>9.2f} {m[7]:>9.2f}"
            )
        print()

    return per_sample_roles, km.cluster_centers_, scaler


def canonicalize_by_roles(tokens, role_ids, sketch_features=None,
                          verbose=False, tag=""):
    """Sketch 청크를 role_id 오름차순으로 정렬.
    Tie-break: (x_centroid, y_centroid).
    """
    tokens = np.asarray(tokens, dtype=np.int32)
    chunks, tail = _split_sketch_chunks(tokens)
    if len(chunks) <= 1 or not role_ids:
        return tokens.copy(), False, None
    if len(role_ids) != len(chunks):
        return tokens.copy(), False, None

    keys = []
    for k, rid in enumerate(role_ids):
        r = 999 if rid is None else int(rid)
        if sketch_features and k < len(sketch_features) and sketch_features[k] is not None:
            x = float(sketch_features[k][0])
            y = float(sketch_features[k][1])
        else:
            x = 0.0; y = 0.0
        keys.append((r, x, y))
    ...
    sorted_chunks = [chunks[i] for i in order]
    out_parts = sorted_chunks + ([tail] if tail.size else [])
    ...
    if verbose:
        roles_orig = [role_ids[i] for i in range(len(chunks))]
        roles_sorted = [role_ids[i] for i in order]
        print(
            f"    role-canon [{tag}] "
            f"roles_orig={roles_orig} → sorted={roles_sorted} "
...
# ══════════════════════════════════════════════════════════════
# ★ Variant discovery & snap (이 파일에서 새로 추가)
# ══════════════════════════════════════════════════════════════
def _chunk_topology_signature(chunk_tokens):
    """sketch chunk 의 topology 지문 = (n_lines, n_arcs, n_circles, n_loops).

    SOL 토큰 개수로 loop 수 추정 (DeepCAD: 각 loop 가 SOL 로 시작).
    """
    cmd = chunk_tokens[:, 0]
    n_line = int((cmd == LINE).sum())
    n_arc = int((cmd == ARC).sum())
    n_circle = int((cmd == CIRCLE).sum())
    n_sol = int((cmd == SOL).sum())
    return (n_line, n_arc, n_circle, n_sol)
...
def discover_variants_per_cluster(per_sample_roles, raw_tokens_list, cluster_ids,
    ...
    if verbose:
        ...
        for c_id in sorted(cluster_variants.keys()):
            vs = cluster_variants[c_id]
            total = sum(v["count"] for v in vs)
            print(f"    cluster {c_id}: {len(vs)} variant(s), total {total} chunks")
            for v in vs:
                sig = v["signature"]
                print(
                    f"      sig=(L{sig[0]} A{sig[1]} C{sig[2]} S{sig[3]})"
                    f"  count={v['count']}"
                )
        print()
...
def snap_decoded_to_variants(
    ...
    for i, ch in enumerate(chunks):
        if i >= len(decoded_feats) or decoded_feats[i] is None:
            out_chunks.append(ch)
            n_unassigned += 1
            details.append(f"sk{i}: no-feat → kept as-is")
            continue
        feat = decoded_feats[i]
        ...
        if nn_d >= max_assign_dist or nn_c not in cluster_variants:
            out_chunks.append(ch)
            n_unassigned += 1
            details.append(
                f"sk{i}: unassigned (nearest c={nn_c} d={nn_d:.2f}) → kept as-is"
            )
            continue

        variants = cluster_variants[nn_c]
        if not variants:
            out_chunks.append(ch)
            n_unassigned += 1
...
def visualize_inverse_z_on_pca(
    ...
    traj_pca = np.stack([pca.transform(z_snap) for z_snap in z_trajectory], axis=0)
    z_tsne_train = None
    try:
        from sklearn.manifold import TSNE
        if N >= 10:
    ...
    if z_tsne_train is not None:
        ...
        axes = [ax_only]
    fig.suptitle(
        f"Inverse design latent trajectory  —  projected onto training PCA  "
        f"({n_starts} starts, {n_snaps} snapshots)",
    ...
    for ti in range(n_types):
        ...
        ax.scatter(z_train_pca[m, 0], z_train_pca[m, 1],
    ...
    if z_tsne_train is not None:
        ...
        for ti in range(n_types):
            ...
            if not m.any(): continue
            tname = type_names[ti] if type_names and ti < len(type_names) else f"type{ti}"
            ax.scatter(z_tsne_train[m, 0], z_tsne_train[m, 1],
        ...
        ax.legend(fontsize=8, loc="best", framealpha=0.85)
    plt.tight_layout()
    print(f"  ✓ inverse z trajectory figure 생성 (n_starts={n_starts}, n_snaps={n_snaps})")
...
def visualize_decoded_structure(
    ...
    if dq_meta is not None and n_ext > 0:
        ...
        # cleanup + enforce + snap 후 EXT 개수 재계산
        n_ext = int((recon_trim[:, 0] == EXT).sum())
