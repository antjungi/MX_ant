...
# -*- coding: utf-8 -*-
"""
DeepCAD AE + Return-3 Common-Curve Residual Surrogate  —  v2
============================================================

v2 변경점 (v1 = AE_inverse.py):
  - Encoder 에 ★ geometric positional encoding 추가
  - 각 sketch 의 위치 (px, py, pz) + 부피 (scale, e1, e2, log_volume)
    + 부피 순위 (size_rank) 를 Fourier feature 로 인코딩한 뒤
    그 sketch 에 속한 모든 토큰에 더해줌
  - Sketch 의 sequence position 에만 의존하지 않고 sketch 의 의미적
    geometry 를 latent 에 직접 주입 → type 간 sketch order mismatch 완화
  - Decoder 는 그대로 유지 (autoregressive 안정성을 위해 — chicken-egg)
...
class CFG:
    ...
    # ★ v2: geometric positional encoding (encoder side)
...
# ══════════════════════════════════════════════════════════════
# ★ v2: Geometric positional encoding helpers
# ══════════════════════════════════════════════════════════════
N_GEO_FEAT = 8   # [px, py, pz, scale, e1, e2, log_volume, size_rank]


def _extract_sketch_features_per_token(x):
    """각 토큰에 대해 그 토큰이 속한 sketch 의 geometric feature vector 를
    부여한다. EXT 토큰을 스캔해서 그 sketch 가 차지하는 토큰 범위를 파악하고,
    EXT 의 slot 값에서 (px, py, pz, scale, e1, e2) 를 뽑은 뒤
    log_volume, size_rank 를 계산.

    Features (N_GEO_FEAT=8):
      0: px           — sketch x position (normalized 0..1)
      1: py           — sketch y position
      2: pz           — sketch z position
      3: scale_2d     — sketch 2D footprint scale
      4: e1           — extrude distance one
      5: e2           — extrude distance two
      6: log_volume   — log(scale^2 · (e1 + e2) + eps)
      7: size_rank    — 같은 sample 내에서 부피 순위 (0 = 가장 큼, 1 = 가장 작음)

    Args:
        x: (B, L, 17) padded token tensor (float)

    Returns:
        (B, L, N_GEO_FEAT) float tensor on same device as x.
        Padding 위치는 0 으로 둠.
    """
    B, L, _ = x.shape
    device = x.device
    ...
    feats_np = np.zeros((B, L, N_GEO_FEAT), dtype=np.float32)
    ...
    for b in range(B):
        ...
        sample_features = []
        last_end = -1
        for ext_pos in ext_positions:
            start_pos = last_end + 1
            ext_row = x_np[b, ext_pos]

            def _slot(i, default):
                v = int(ext_row[1 + i])
                return float(v) / float(QMAX) if v >= 0 else float(default)

            px = _slot(P_PX, 0.5)
            py = _slot(P_PY, 0.5)
            pz = _slot(P_PZ, 0.5)
            scale = _slot(P_SCALE, 0.5)
            e1 = _slot(P_E1, 0.1)
            e2 = _slot(P_E2, 0.0)
            log_vol = math.log(scale * scale * (e1 + e2) + 1e-6)

            sample_features.append({
                "start": int(start_pos),
                "end": int(ext_pos),
                "vec": [px, py, pz, scale, e1, e2, log_vol],
            })
            last_end = int(ext_pos)

        n_sk = len(sample_features)
        log_vols = np.array([s["vec"][6] for s in sample_features])
        order = np.argsort(-log_vols)  # 가장 큰 부피가 rank 0
        rank_norm = np.zeros(n_sk, dtype=np.float32)
        for r, idx in enumerate(order):
            rank_norm[idx] = float(r) / float(max(n_sk - 1, 1))

        for i, sf in enumerate(sample_features):
            feat_vec = sf["vec"] + [float(rank_norm[i])]
            feats_np[b, sf["start"]:sf["end"] + 1, :] = feat_vec

    return torch.from_numpy(feats_np).to(device)


class GeometricEmbedding(nn.Module):
    """Per-sketch geometric features → d_model embedding.

    Continuous feature 들을 Fourier feature 로 확장한 뒤 MLP 로 d_model 차원으로 사영.
    같은 sketch 에 속한 모든 토큰은 같은 geometric embedding 을 받음.
    """
    def __init__(self, d_model, n_features, n_freqs=6, dropout=0.1):
        super().__init__()
        freqs = (2.0 ** torch.arange(n_freqs, dtype=torch.float32)) * math.pi
        self.register_buffer("freqs", freqs)
        in_dim = n_features * (1 + 2 * n_freqs)   # raw + sin/cos
        hid = max(d_model * 2, 128)
    ...
    def forward(self, feats):
        # feats: (B, L, F)
        B, L, F = feats.shape
        ang = feats.unsqueeze(-1) * self.freqs.view(1, 1, 1, -1)  # (B, L, F, n_freqs)
        sin_f = torch.sin(ang).reshape(B, L, F * self.freqs.size(0))
        cos_f = torch.cos(ang).reshape(B, L, F * self.freqs.size(0))
        enc = torch.cat([feats, sin_f, cos_f], dim=-1)
        return self.mlp(enc)
...
class DeepCADBaselineAE(nn.Module):
    def __init__(
        ...
        self.use_geometric_pe = bool(use_geometric_pe)
        ...
        # ★ v2: Geometric PE (encoder 전용)
        if self.use_geometric_pe:
            self.geo_emb = GeometricEmbedding(
                d_model=d_model,
                n_features=N_GEO_FEAT,
                n_freqs=int(geo_pe_n_freqs),
                dropout=float(geo_pe_dropout),
            )
            self.geo_post_norm = nn.LayerNorm(d_model)
        else:
            self.geo_emb = None
    ...
    def encode(self, x, geo_feats=None):
        ...
        # ★ v2: Geometric embedding 추가 (sketch 의 위치/부피)
        if self.use_geometric_pe and self.geo_emb is not None:
            if geo_feats is None:
                geo_feats = _extract_sketch_features_per_token(x)
            geo_e = self.geo_emb(geo_feats)
            geo_e = geo_e * (~pad_mask).unsqueeze(-1).float()
            emb = self.geo_post_norm(emb + geo_e)
