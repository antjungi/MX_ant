...
# -*- coding: utf-8 -*-
"""
DeepCAD AE + Return-3 Common-Curve Residual Surrogate (body-set variant)
=========================================================================

AE_inverse.py 기반. body(=SOL...EXT) 단위 순서 의존성만 제거.

추가 사항:
  - JointDataset / 모델 구조 / surrogate / inverse design 코드는 그대로
  - run_epoch 에서 train_mode 일 때 batch 단위로 body 순서를 랜덤하게 섞음
    → 인코더가 같은 형상에 대해 순서에 무관한 z 를 학습하도록 augmentation
  - cfg.use_consistency=True 이면 z(x) ≈ z(shuffle(x)) consistency loss 추가
    (1회 추가 encoder forward; AR 디코더는 안 거침)
"""
...
class CFG:
    ...
    # body-set augmentation
    body_shuffle_aug: bool = True
...
# ── body-set augmentation utilities ────────────────────────────
def split_bodies(tokens):
    """
    tokens: [L, 17] int array (no EOS handling needed by caller; we trim).
    returns list of body arrays [L_i, 17] where each body starts with SOL.
    EXT 으로 끝난 정상 body 만 모은다. 비정상(EXT 없음) body 는 버린다.
    """
    t = trim_after_eos(tokens)
    bodies = []
    cur = None
    for row in t:
        cmd = int(row[0])
...
def shuffle_bodies_tokens(tokens_padded, max_len, rng=None):
    """
    tokens_padded: [max_len, 17] float/int (PAD_V padded, has EOS).
    body 순서를 랜덤 셔플 후 다시 same shape padded 로 반환.
    body 가 2개 미만이면 그대로 반환.
    """
    if rng is None:
        rng = random

    t = np.asarray(tokens_padded).astype(np.int32)
    bodies = split_bodies(t)
    if len(bodies) < 2:
    ...
    new_t = np.concatenate(bodies, axis=0)
    eos_row = np.full((1, 17), PAD_V, dtype=np.int32)
    eos_row[0, 0] = EOS
    new_t = np.concatenate([new_t, eos_row], axis=0)

    if new_t.shape[0] > max_len:
        new_t = new_t[:max_len].copy()
        new_t[-1, :] = PAD_V
        new_t[-1, 0] = EOS
    else:
        pad = np.full((max_len - new_t.shape[0], 17), PAD_V, dtype=np.int32)
        new_t = np.concatenate([new_t, pad], axis=0)

    out = new_t.astype(np.asarray(tokens_padded).dtype, copy=False)
    return out
...
def run_epoch(ae, mlp, loader, optimizer, device, cfg, interp_w, train_mode=True):
    ...
    for batch in loader:
        ...
        with torch.set_grad_enabled(train_mode):
            ...
            # consistency loss: encode again with a different shuffle
