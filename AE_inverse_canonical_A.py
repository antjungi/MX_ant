...
# -*- coding: utf-8 -*-
"""
DeepCAD AE + Return-3 Common-Curve Residual Surrogate  —  canonical Plan A
==========================================================================

Plan A 변경점 (base = AE_inverse_canonical.py):
  - 정렬 키: (z_bucket, -footprint_area, x, y)
      · z_bucket = round(z_centroid / Z_BUCKET_SIZE)
      · 같은 z layer 에 있는 sketch 들은 같은 bucket → 같은 role 그룹
      · bucket 안에서만 면적 desc 로 정렬
  - 효과: type 별 sketch 수가 달라도 "layer 별 role" 이 유지됨
      · 예) Type 1 [GND, sub, patch, feed] vs Type 2 [GND, sub, patch, slot, feed]
        둘 다 처음 GND/sub/patch 까지 같은 layer 그룹, 그 다음에서 차이
  - 단점: bucket 안의 sketch 수가 type 별로 다르면 여전히 어긋남 (Plan B 가 해결)

목적:
  "비슷한 z 평면 + 비슷한 크기의 sketch 는 비슷한 sequence 위치"
"""
...
Z_BUCKET_SIZE = 0.5   # mm — Plan A: z layer bucket 크기


def _compute_canonical_keys_from_json(json_data, z_bucket_size=Z_BUCKET_SIZE):
    """각 sketch 의 정렬 키를 JSON 실좌표로 계산. (Plan A: z bucket)

    key = (z_bucket, -footprint_area, x_centroid, y_centroid)
      z_bucket = round(z_centroid / z_bucket_size)
    같은 z layer (bucket) 안의 sketch 끼리만 면적 desc.
    다른 layer 는 bucket 이 다르므로 자연스럽게 layer 단위로 그룹됨.
    ...
    for sk in sketches:
        ...
        keys.append((z_bucket, -footprint, cx, cy))
        details.append(
            f"z_bucket={z_bucket:3d}  z={cz:7.2f}mm  "
            f"footprint={footprint:8.2f}mm²  "
