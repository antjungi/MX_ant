...
set-based 변경점 (base = AE_inverse.py):
  - Decoder 가 DETR 스타일 **set-based**.
    · K 개 sketch query slot (learnable) 이 cross-attention 으로 z 를 봄
    · 각 slot 마다 (sketch_latent, is_active, summary) 출력
    · 각 sketch_latent 을 autoregressive sub-decoder 가 토큰열로 풀이
  - 두 레벨 분리:
    · Inter-sketch: 순서 무관 set (slot 간 의존성 없음)
    · Intra-sketch: 여전히 autoregressive (SOL→LINE→...→EXT 의존성 보존)
  - 학습: Hungarian matching (summary 거리 기반) → matched slot 에 token CE
  - 추론: 활성 slot 만 모아 토큰열 연결

기대:
  - Sketch 순서 mismatch 본질적 해결
  - Type 별 sketch 수가 달라도 K slot 에 자동 할당
  - Latent 공간에서 의미적 sketch 단위 표현 → inverse design 안정
...
N_SKETCH_SLOTS = 16              # K — 동시에 출력 가능한 sketch 최대 수
MAX_TOKENS_PER_SKETCH = 48       # 한 sketch 의 최대 토큰 수 (SOL ... EXT 포함)
SUMMARY_DIM = 4                  # (z, x, y, log_area) — Hungarian matching 용
...
class CFG:
    ...
    # aux numeric (set-based 에선 사용 안 함)
    ...
    w_aux: float = 2.0
    w_sparam: float = 5.0
    w_active: float = 0.5       # ★ set-based: is_active BCE 가중치
    w_summary: float = 1.0      # ★ set-based: summary L2 regression 가중치
...
def _ext_row_summary(ext_row):
    """EXT 토큰 row 에서 4-dim summary (z, x, y, log_area) 추출."""
...
def split_tokens_into_sketches(t, max_tokens_per_sketch, n_slots):
    """전체 token 시퀀스 → per-sketch padded tokens + summary + count.

    Args:
        t: (L_total, 17) int32 array (canonical token sequence)
        max_tokens_per_sketch: 한 sketch 의 최대 토큰 수
        n_slots: 출력 slot 개수 (K)

    Returns:
        sk_tokens: (K, max_tokens_per_sketch, 17) int32, padding=PAD_V
        sk_summary: (K, SUMMARY_DIM) float32
        n_sketches: int, 실제 sketch 수 (≤ K)
    """
    ...
    for i in range(len(t)):
        c = int(cmd_seq[i])
        if c == EXT:
            chunk = t[start:i + 1].copy()
            sketch_chunks.append(chunk)
            start = i + 1
        elif c == EOS or c < 0:
            break
    ...
    for k in range(n_sk):
        ...
        # EXT row 가 마지막에 있어야 summary 정확 — chunk 의 마지막 row
        ext_row = chunk[-1]
        sk_summary[k] = _ext_row_summary(ext_row)
...
class JointDataset(Dataset):
    def __init__(
        ...
        self.raw = []
        self.padded = []
        # ★ set-based 용 per-sample 미리 split
        self.sk_tokens = []      # 각 (K, max_tok_per_sk, 17)
        self.sk_summary = []     # 각 (K, SUMMARY_DIM)
        self.sk_count = []       # 각 int (활성 sketch 수)
        ...
        for f in self.npy_files:
            ...
            # Check for truncated sketches
            for k in range(n_sk):
                cmd_first = int(sk_t[k, 0, 0])
                cmd_at_max = int(sk_t[k, min(self.max_tokens_per_sketch - 1, len(sk_t[k]) - 1), 0])
                if cmd_at_max != EXT and cmd_first != PAD_V:
                    # EXT missing at end → truncated
                    n_truncated_sk += 1
                    break

        if n_truncated_sk > 0:
            print(
                f"  ⚠ {n_truncated_sk} samples 가 max_tokens_per_sketch="
                f"{self.max_tokens_per_sketch} 보다 큰 sketch 를 가짐 (truncated)"
            )
    ...
    def __getitem__(self, idx):
        return (
            torch.tensor(self.padded[idx], dtype=torch.float32),           # 0: encoder 입력 (전체 시퀀스)
            ...
            torch.tensor(int(self.sk_count[idx]), dtype=torch.long),        # 6: 실제 sketch 수
        )
...
class DeepCADBaselineAE(nn.Module):
    """Set-based AE: Encoder → z → SetDecoder → K slot 의 (sk_latent, is_active,
    summary) → PerSketchDecoder 가 각 sk_latent 을 autoregressive 로 토큰 풀이.
    """
    def __init__(
        self,
        max_len,
        ...
        self.n_slots = int(n_slots)
        self.max_tokens_per_sketch = int(max_tokens_per_sketch)
        self.sketch_latent_dim = d_model

        # ── Token embedding (shared by encoder + per-sketch decoder) ──
        ...
        self.set_decoder = nn.TransformerDecoder(
            set_dec_layer, num_layers=set_dec_n_layers,
            norm=nn.LayerNorm(d_model),
        )
        self.sketch_queries = nn.Parameter(
            torch.randn(self.n_slots, d_model) * 0.02
        )
        self.is_active_head = nn.Linear(d_model, 1)
        self.summary_head = nn.Linear(d_model, SUMMARY_DIM)

        # ── ★ Per-sketch token decoder (shared across slots) ──
        self.sk_to_mem = nn.Linear(d_model, mem_tokens * d_model)
        self.sk_pos_enc = SinPosEnc(
            d_model, max_len=self.max_tokens_per_sketch + 4, dropout=dropout,
        )
        ...
        # Aux numeric disabled in set-based (output size impractical)
    ...
    def decode_set(self, z):
        """z → (sk_latents, is_active_logits, summaries).
        z: (B, latent)
        Returns:
          sk_latents: (B, K, d_model)
          is_active_logits: (B, K)
          summaries: (B, K, SUMMARY_DIM)
        """
        B = z.size(0)
        ...
        # sk_latents = encoder output (d_model 차원)
        return out, is_active, summaries

    def decode_per_sketch_teacher(self, sk_latents, target_sk_tokens):
        """Per-slot teacher-forced decoding (training).
        sk_latents: (B, K, d_model) — reordered to match target
        target_sk_tokens: (B, K, L_sk, 17)
        Returns:
          cmd_logits: (B, K, L_sk, N_CMD)
          prm_logits: (B, K, L_sk, N_ARGS, N_QUANT)
        """
        ...
        tgt_pad = torch.cat([
            torch.zeros((BK, 1), dtype=torch.bool, device=device),
            prev_pad,
        ], dim=1)
        ...
        prm_logits = self.param_head(out).reshape(
            B, K, L_sk, N_ARGS, N_QUANT,
        )
    ...
    def _gen_one_sketch(self, sk_lat):
        """Generate one sketch's tokens autoregressively.
        sk_lat: (1, d_model)
        Returns: (L_sk, 17) numpy int32 array, ending at EXT (if reached) or max_tok
        """
    ...
    def generate(self, z, max_gen_len=None):
        """Set-based generation: K slot → activate → autoregress per slot → concat.
        z: (B, latent)
        Returns: (B, L_total, 17) long tensor.
        """
        B = z.size(0)
        device = z.device

        sk_latents, is_active_logits, _summaries = self.decode_set(z)
        ...
        for b in range(B):
            ...
            chunks = []
            for k in active_idx:
                sk_lat = sk_latents[b:b + 1, k]  # (1, d_model)
                tok = self._gen_one_sketch(sk_lat)
                # Sketch 가 EXT 로 끝나지 않으면 강제로 추가
                if tok.shape[0] == 0:
                    continue
                if int(tok[-1, 0]) != EXT:
            ...
            if flat.shape[0] < max_gen_len:
                pad = np.full(
                    (max_gen_len - flat.shape[0], 17), PAD_V, dtype=np.int32,
                )
...
# Losses
# ══════════════════════════════════════════════════════════════
def hungarian_match_summaries(pred_summaries, target_summaries, n_sketches):
    """Hungarian matching between predicted slot summaries and target sketch summaries.

    pred_summaries: (B, K, SUMMARY_DIM)
    target_summaries: (B, K, SUMMARY_DIM) — first n_sketches[b] are valid
    n_sketches: (B,) long

    Returns:
      matching: (B, K) long — matching[b, k] = target_idx (0..n_sk-1) 또는 -1
    """
    try:
        from scipy.optimize import linear_sum_assignment
    except ImportError:
        # Fallback: identity (no matching)
    ...
    for b in range(B):
        ...
        if n_sk == 0:
            continue
        cost = np.linalg.norm(
            pred_np[b, :, None, :] - target_np[b, None, :n_sk, :],
            axis=-1,
        )  # (K, n_sk)
...
def reorder_targets_by_matching(target_sk_tokens, target_summaries, matching):
    """matching 에 따라 target sketch 들을 slot 순서로 재배치.

    target_sk_tokens: (B, K, L_sk, 17)
    target_summaries: (B, K, SUMMARY_DIM)
    matching: (B, K) — slot k → target idx 또는 -1

    Returns:
      reord_tokens: (B, K, L_sk, 17) — matching 된 slot 만 채워짐, 나머지는 PAD
      reord_summaries: (B, K, SUMMARY_DIM)
      slot_matched: (B, K) bool — matched 여부
    """
    B, K, L_sk, _ = target_sk_tokens.shape
    device = target_sk_tokens.device
    ...
    return reord_tokens, reord_summaries, slot_matched


def set_based_ae_loss(
    ae,
    sk_latents, is_active_logits, pred_summaries,
    target_sk_tokens, target_summaries, n_sketches,
    cfg,
):
    """Set-based AE loss.
    1) Hungarian matching: pred_summaries vs target_summaries (n_sketches 까지)
    2) target 재배치 후 ae.decode_per_sketch_teacher 로 cmd/prm logits 계산
    3) Losses:
       - matched slot 토큰 CE (cmd + prm)
       - is_active BCE
       - summary L2 regression
    """
    B, K, L_sk, _ = target_sk_tokens.shape
    device = target_sk_tokens.device
    ...
    with torch.no_grad():
        matching = hungarian_match_summaries(
            pred_summaries, target_summaries, n_sketches,
        )

    # 2) Reorder targets by matching
    reord_tokens, reord_summaries, slot_matched = reorder_targets_by_matching(
        target_sk_tokens, target_summaries, matching,
    )

    # 3) Decode per sketch teacher
    cmd_logits, prm_logits = ae.decode_per_sketch_teacher(
        sk_latents, reord_tokens,
    )

    # Token CE loss (matched slots only)
    target_cmd = reord_tokens[..., 0].long()       # (B, K, L_sk)
    cmd_valid = (target_cmd >= 0) & slot_matched.unsqueeze(-1)
    target_cmd_c = target_cmd.clamp(0, N_CMD - 1)
    cmd_ce = F.cross_entropy(
        cmd_logits.reshape(-1, N_CMD),
        target_cmd_c.reshape(-1),
        reduction="none",
    ).reshape(B, K, L_sk)
    cmd_loss = (cmd_ce * cmd_valid.float()).sum() / (cmd_valid.float().sum() + 1e-8)

    target_prm = reord_tokens[..., 1:].long()      # (B, K, L_sk, N_ARGS)
    prm_valid = (target_prm >= 0) & slot_matched.view(B, K, 1, 1)
    target_prm_c = target_prm.clamp(0, N_QUANT - 1)
    prm_ce = F.cross_entropy(
        prm_logits.reshape(-1, N_QUANT),
        target_prm_c.reshape(-1),
        reduction="none",
    ...
    active_loss = F.binary_cross_entropy_with_logits(
        is_active_logits, is_active_gt,
    )

    # Summary L2 on matched slots
    sum_mask = slot_matched.unsqueeze(-1).float()
    summary_sq = (pred_summaries - reord_summaries) ** 2 * sum_mask
    summary_loss = summary_sq.sum() / (
        sum_mask.sum() * SUMMARY_DIM + 1e-8
    )

    w_active = getattr(cfg, "w_active", 0.5)
    w_summary = getattr(cfg, "w_summary", 1.0)
...
# Legacy alias for backward-compat call sites (not actually used set-based)
def ae_loss_fn(cmd_logits, prm_logits, aux_numeric, target, cfg):
    """Set-based 버전에선 사용하지 않음 (호환용 stub)."""
    raise NotImplementedError(
        "ae_loss_fn is replaced by set_based_ae_loss in set-based version"
    )
...
def run_epoch(ae, mlp, loader, optimizer, device, cfg, interp_w, train_mode=True):
    ...
    for batch in loader:
        # ★ set-based: 7-tuple (tok, sparam_sel, sparam_full, type, sk_tokens, sk_summary, sk_count)
        (batch_tok, batch_sel_db, batch_full_db, _tid,
         batch_sk_tokens, batch_sk_summary, batch_n_sk) = batch
...
def evaluate_sparam_predictions(ae, mlp, dataset, val_idx_per_type, type_names, cfg, device):
    ...
    for ti, tname in enumerate(type_names):
        ...
        for idx in idxs:
            tok, true_sel_db, true_full_db, _tid, _sk_t, _sk_s, _n_sk = dataset[idx]
...
def train_fixed_medium_var(
    ...
    print(f"  Aux numeric         : disabled (set-based)")
    print(f"  Set-based decoder   : n_slots={getattr(cfg, 'n_slots', N_SKETCH_SLOTS)}, "
          f"max_tok_per_sketch={getattr(cfg, 'max_tokens_per_sketch', MAX_TOKENS_PER_SKETCH)}, "
          f"set_dec_layers={getattr(cfg, 'set_dec_n_layers', 3)}")
    print(f"  Set-based weights   : w_active={getattr(cfg, 'w_active', 0.5)}, "
          f"w_summary={getattr(cfg, 'w_summary', 1.0)}")
