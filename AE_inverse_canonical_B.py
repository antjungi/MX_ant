...
# -*- coding: utf-8 -*-
"""
DeepCAD AE + Return-3 Common-Curve Residual Surrogate  —  canonical Plan B
==========================================================================

Plan B 변경점 (base = AE_inverse_canonical.py):
  - z layer bucket 별로 슬롯을 미리 정의 + 부족하면 ★ NULL sketch 로 패딩 ★

절차:
  (1) 학습 데이터 전체를 pre-scan → 각 z bucket 의 max sketch 수 구함
      예) bucket 0(GND layer): max 1, bucket 1(sub): 1, bucket 2: 4, ...
  (2) 그 max 들을 합쳐서 fixed slot layout 생성 (총 N slots)
      예) slot 0: bucket0 #1, slot 1: bucket1 #1, slot 2~5: bucket2 #1~#4, ...
  (3) 각 sample 의 sketch 들을 자기 bucket 의 slot 에 면적 desc 순으로 배치
  (4) 빈 slot 은 ★ NULL sketch (SOL+EXT with zero params) ★ 로 채움
  (5) 모든 sample 이 fixed N slot 으로 통일

효과:
  - Slot k 가 모든 type 에서 같은 role
  - Position-divergent 문제 본질적으로 해결
  - Type 별 sketch 수 차이는 NULL 로 흡수

단점:
  - 시퀀스 길이 증가 (NULL slot 추가됨)
  - Decoder 가 NULL 토큰도 학습해야 함
  - Inverse design 출력에서 NULL slot 은 무시하고 렌더해야 함
"""
...
def _compute_canonical_keys_from_json(json_data):
    """각 sketch 의 정렬 키를 JSON 실좌표로 계산.

    key = (z_centroid, -footprint_area, x_centroid, y_centroid)
    json_to_real_sketches() 로 loop 점들을 3D 실좌표 (mm) 로 변환한 뒤
    bbox 의 큰 두 축 = sketch plane footprint area.

    Returns: (keys, details) — 정렬 키 리스트와 사용자 친화적 설명 문자열.
              실패 시 (None, None).
    """
    try:
    ...
    for sk in sketches:
        ...
        keys.append((cz, -footprint, cx, cy))
        details.append(
            f"z={cz:7.2f}mm  footprint={footprint:8.2f}mm²  "
            f"bbox={bbox_ext[0]:5.2f}×{bbox_ext[1]:5.2f}×{bbox_ext[2]:5.2f}"
        )
    return keys, details


def canonicalize_sketches(tokens, json_file=None, verbose=False, tag=""):
    """Sketch 청크 (SOL ... EXT) 순서를 canonical 정렬.
    ...
    if json_file is not None and os.path.exists(json_file):
        try:
            ...
            with open(json_file, "r", encoding="utf-8") as f:
                jd = _json.load(f)
            keys, details = _compute_canonical_keys_from_json(jd)
        except Exception:
            keys = None

    if keys is None or len(keys) != len(chunks):
        if verbose:
            print(
                f"    canon [{tag}] SKIP — JSON keys not usable "
    ...
    if verbose:
        print(
            f"    canon [{tag}] n_sk={len(chunks)} "
            f"new_order={order} "
            f"{'(REORDERED)' if changed else '(no change)'}"
        )
        for i, det in enumerate(details):
            pos = order.index(i) + 1
            mark = "→ pos1" if pos == 1 else f"  pos{pos}"
            print(f"      orig_sk[{i}] {mark}  {det}")
...
def _reorder_json_sequence_pairs(json_data, perm):
    ...
    Plan B 에서 perm 은 NULL slot 표시 (-1) 를 포함할 수 있음.
    -1 은 skip (JSON 에는 NULL sketch 가 없음).

    perm[k] = original sketch index 또는 -1 (NULL).
    """
    try:
    ...
    # perm 에서 -1 (NULL) 을 제외한 실제 index 만 추출
    real_indices = [p for p in perm if p is not None and p >= 0]
    if not real_indices:
        return json_data
    # real index 들이 모두 valid 한지 확인
    if any(p >= len(pairs) for p in real_indices):
        return json_data

    reordered_pairs = [pairs[p] for p in real_indices]
    new_seq = []
...
# ══════════════════════════════════════════════════════════════
# ★ Plan B: role-aware slot layout + NULL padding
# ══════════════════════════════════════════════════════════════
Z_BUCKET_SIZE_B = 0.5   # mm — z layer bucket 크기


def _make_null_sketch_chunk():
    """NULL sketch: SOL + EXT (모든 param 0).
    Geometry 영향 없음. Decoder 는 'NULL' 토큰 패턴으로 학습 가능."""
    chunk = np.full((2, 17), PAD_V, dtype=np.int32)
    # SOL row
    chunk[0, 0] = SOL
    # EXT row (모든 param 슬롯 = 0, scale=0 → 영향 없는 sketch)
    chunk[1, 0] = EXT
...
def _is_null_sketch(chunk):
    """chunk (예: SOL ... EXT) 가 NULL sketch 인지 판별.
    조건: SOL 직후 바로 EXT 가 있고 EXT 의 SCALE, E1, E2 가 모두 0.
    """
    if chunk is None or chunk.shape[0] < 2:
        return False
    # 첫 row 가 SOL 이어야
    if int(chunk[0, 0]) != SOL:
        return False
    # 마지막 row 가 EXT 여야
    last = chunk[-1]
    if int(last[0]) != EXT:
        return False
    # 중간에 LINE/ARC/CIRCLE 이 거의 없어야 (있어도 0 개)
    if chunk.shape[0] > 2:
        for r in range(1, chunk.shape[0] - 1):
            c = int(chunk[r, 0])
            if c in (LINE, ARC, CIRCLE):
                return False
    # EXT 의 SCALE/E1/E2 가 모두 0
    s = int(last[1 + P_SCALE]) if last[1 + P_SCALE] >= 0 else 0
    e1 = int(last[1 + P_E1]) if last[1 + P_E1] >= 0 else 0
    e2 = int(last[1 + P_E2]) if last[1 + P_E2] >= 0 else 0
    return s == 0 and e1 == 0 and e2 == 0


def _compute_sketch_z_bucket(json_data, z_bucket_size=Z_BUCKET_SIZE_B):
    """각 sketch 의 (z_bucket, footprint_area) 반환.
    Returns: list of (z_bucket, footprint_area) 또는 None.
    """
    try:
        sketches = json_to_real_sketches(json_data)
    except Exception:
    ...
    for sk in sketches:
        ...
        centroid = arr.mean(axis=0)
        cz = float(centroid[2])
        z_bucket = int(round(cz / max(z_bucket_size, 1e-6)))
        info.append((z_bucket, footprint))
    return info


def build_slot_layout(npy_files, json_files, z_bucket_size=Z_BUCKET_SIZE_B):
    """학습 데이터 전체를 pre-scan 해서 bucket 별 max sketch 수 결정.

    Returns:
        slot_buckets: list of bucket_id, 각 slot 이 어느 bucket 에 속하는지.
                      예) [-1, -1, 0, 0, 1, 2, 2, 2] → bucket -1: 2 slot, 0: 2 slot, ...
                      이 길이가 total slot 개수.
        bucket_max_counts: dict {bucket_id: max_count}
    """
    bucket_max_counts = {}
    n_processed = 0
    for jf in json_files:
        ...
        try:
            ...
            with open(jf, "r", encoding="utf-8") as f:
                jd = _json.load(f)
            info = _compute_sketch_z_bucket(jd, z_bucket_size=z_bucket_size)
            if info is None:
                continue
        except Exception:
            continue
        ...
        # bucket 별 count 누적
        bucket_count = {}
        for bid, _area in info:
            bucket_count[bid] = bucket_count.get(bid, 0) + 1
        # max 업데이트
        for bid, cnt in bucket_count.items():
            bucket_max_counts[bid] = max(bucket_max_counts.get(bid, 0), cnt)

    if not bucket_max_counts:
        return [], {}

    # bucket id 오름차순 정렬, 각 bucket 의 max_count 만큼 slot 할당
    sorted_buckets = sorted(bucket_max_counts.keys())
    slot_buckets = []
    for bid in sorted_buckets:
        slot_buckets.extend([bid] * bucket_max_counts[bid])

    return slot_buckets, bucket_max_counts


def canonicalize_sketches_with_slots(
    tokens, json_file, slot_buckets, z_bucket_size=Z_BUCKET_SIZE_B,
    verbose=False, tag="",
):
    """Plan B 의 핵심: 토큰을 fixed slot layout 으로 정렬 + NULL 패딩.

    Args:
        tokens: (L_total, 17) 원본 토큰
        json_file: JSON path
        slot_buckets: build_slot_layout 의 첫번째 반환값. 각 slot 이 어느 bucket 인지.

    Returns:
        out_tokens: (L_new, 17) — slot 별 sketch 들이 순서대로, NULL 포함
        perm: list — slot 별 원본 sketch index (NULL slot 은 -1)
        was_changed: bool
    """
    tokens = np.asarray(tokens, dtype=np.int32)
    chunks, tail = _split_sketch_chunks(tokens)
    if not chunks or not slot_buckets:
        return tokens.copy(), None, False

    # JSON 으로 각 chunk 의 (bucket, area) 추정
    try:
        import json as _json
        with open(json_file, "r", encoding="utf-8") as f:
            jd = _json.load(f)
        info = _compute_sketch_z_bucket(jd, z_bucket_size=z_bucket_size)
        if info is None or len(info) != len(chunks):
            return tokens.copy(), None, False
    except Exception:
        return tokens.copy(), None, False

    # bucket 별로 chunk 그룹화 + 면적 desc sort
    bucket_to_chunks = {}  # bucket_id → list of (orig_idx, area)
    for orig_idx, (bid, area) in enumerate(info):
        bucket_to_chunks.setdefault(bid, []).append((orig_idx, area))
    for bid in bucket_to_chunks:
        bucket_to_chunks[bid].sort(key=lambda t: -t[1])  # area desc

    # slot 채우기
    null_chunk = _make_null_sketch_chunk()
    result_chunks = []
    perm = []
    bucket_ptr = {bid: 0 for bid in bucket_to_chunks}

    for slot_idx, slot_bid in enumerate(slot_buckets):
        # 이 slot 의 bucket 에 남은 chunk 가 있는지
        if slot_bid in bucket_to_chunks and bucket_ptr[slot_bid] < len(bucket_to_chunks[slot_bid]):
            orig_idx, _area = bucket_to_chunks[slot_bid][bucket_ptr[slot_bid]]
            bucket_ptr[slot_bid] += 1
            result_chunks.append(chunks[orig_idx])
            perm.append(orig_idx)
        else:
            # NULL 패딩
            result_chunks.append(null_chunk.copy())
            perm.append(-1)

    # 만약 sample 의 sketch 수가 slot 수보다 많으면 (rare, layout 부족) 추가 append
    used = set(p for p in perm if p >= 0)
    extras = [i for i in range(len(chunks)) if i not in used]
    if extras:
        # bucket 정보를 무시하고 면적 desc 로 그냥 뒤에 추가
        extras.sort(key=lambda i: -info[i][1])
        for ei in extras:
            result_chunks.append(chunks[ei])
            perm.append(ei)

    # 재조합
    out = np.concatenate(result_chunks + ([tail] if tail.size else []), axis=0)

    was_changed = (perm != list(range(len(chunks))))

    if verbose:
        n_null = sum(1 for p in perm if p == -1)
        print(
            f"    canon-B [{tag}] orig_n_sk={len(chunks)}, "
            f"out_n_slots={len(perm)} (NULL={n_null}) "
            f"{'(CHANGED)' if was_changed else '(no change)'}"
        )
        for slot_i, p in enumerate(perm[:min(12, len(perm))]):
            if p == -1:
                print(f"      slot[{slot_i}] bucket={slot_buckets[slot_i] if slot_i < len(slot_buckets) else '?'}  NULL")
            else:
                bid, area = info[p]
                print(f"      slot[{slot_i}] bucket={bid}  ← orig_sk[{p}] area={area:.2f}mm²")

    return out, perm, was_changed
...
class JointDataset(Dataset):
    def __init__(
        ...
        # type 별 첫 sample 1 개씩만 verbose 출력 (정렬 동작 확인용)
        type_ids_arr = (
            np.asarray(type_ids, dtype=np.int64)
            if type_ids is not None else
            np.zeros(len(self.npy_files), dtype=np.int64)
        )
        seen_types = set()

        # ★ Plan B: 학습 데이터 전체를 pre-scan → slot layout 결정
        self.slot_buckets = []
        self.bucket_max_counts = {}
        if self.do_canonicalize:
            print("  ★ Plan B: pre-scanning JSON files to build slot layout...")
            self.slot_buckets, self.bucket_max_counts = build_slot_layout(
                self.npy_files, self.json_files,
                z_bucket_size=Z_BUCKET_SIZE_B,
            )
            print(
                f"    bucket counts: {dict(sorted(self.bucket_max_counts.items()))}"
            )
            print(f"    total slots: {len(self.slot_buckets)} "
                  f"(slot→bucket: {self.slot_buckets[:16]}{'...' if len(self.slot_buckets) > 16 else ''})")

        self.raw = []
        self.padded = []
        self.canon_perms = []        # ★ per-slot perm: -1 if NULL
        n_changed = 0
        n_total_canon = 0
        n_skip = 0
        ...
        for i, f in enumerate(self.npy_files):
            ...
            perm = None
            if self.do_canonicalize and self.slot_buckets:
                ...
                if jf is not None and os.path.exists(jf):
                    t_canon, perm, was_changed = canonicalize_sketches_with_slots(
                        t,
                        json_file=jf,
                    ...
                    if was_changed:
                        n_changed += 1
                    # NULL slot 패딩으로 토큰 길이 늘어났을 수 있음 → max_len 까지 truncate
                    if t_canon.shape[0] > max_len:
                        t_canon = ensure_eos_when_truncated(t_canon, max_len)
                    t = t_canon
                else:
                    n_skip += 1
                    if verbose:
                        print(f"    canon-B [sample{i}/type{tid}] SKIP — no JSON")
            self.raw.append(t)
        ...
        if self.do_canonicalize:
            print(
                f"  ★ Plan B canonical: applied to {n_changed}/{n_total_canon} sequences "
